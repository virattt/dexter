...
import { marketSentimentTool } from './market-sentiment.js';
...
describe('market_sentiment registry gating', () => {
  ...
  test('does not register market_sentiment without ADANOS_API_KEY', () => {
    delete process.env.ADANOS_API_KEY;
    const toolNames = getToolRegistry('gpt-5.4').map((tool) => tool.name);
    expect(toolNames).not.toContain('market_sentiment');
  });

  test('registers market_sentiment when ADANOS_API_KEY is set', () => {
    process.env.ADANOS_API_KEY = 'sk_test_adanos';
    const toolNames = getToolRegistry('gpt-5.4').map((tool) => tool.name);
    expect(toolNames).toContain('market_sentiment');
  });
});
