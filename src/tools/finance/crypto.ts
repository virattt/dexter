...
export const getCryptoPriceSnapshot = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const parsedInput = CryptoPriceSnapshotInputSchema.parse(input);
    const params = { ticker: parsedInput.ticker };
...
export const getCryptoPrices = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const parsedInput = CryptoPricesInputSchema.parse(input);
    const params = {
      ticker: parsedInput.ticker,
      interval: parsedInput.interval,
      interval_multiplier: parsedInput.interval_multiplier,
    ...
    const endDate = new Date(parsedInput.end_date + 'T00:00:00');
