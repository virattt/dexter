...
Get structured cross-source stock sentiment snapshots from Reddit, X/Twitter, and Polymarket.

## When to Use

- Questions like "what's the market sentiment on TSLA?"
...
function buildMarketSentimentPayload(
  tickers: string[],
  days: number,
...
export const marketSentimentTool = new DynamicStructuredTool({
  name: 'market_sentiment',
  description:
    'Retrieves structured cross-source stock sentiment snapshots from Reddit, X/Twitter, and Polymarket. Use for public market sentiment, social sentiment comparisons, and cross-source alignment checks.',
  schema: MarketSentimentInputSchema,
  func: async (input) => {
    try {
      ...
      return formatToolResult(
        buildMarketSentimentPayload(tickers, input.days, requestedSources, successful, failed),
        successful.map((result) => result.url),
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`[market_sentiment] ${message}`);
    }
  },
});
