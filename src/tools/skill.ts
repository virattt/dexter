...
type GetSkillFn = (name: string) => ReturnType<typeof getSkill>;
type DiscoverSkillsFn = () => ReturnType<typeof discoverSkills>;

/**
 * Create a skill invocation tool with injectable dependencies.
 * Primarily useful for testing; production code uses `skillTool`.
 */
export function createSkillTool(
  ...
  return new DynamicStructuredTool({
    ...
    func: async ({ skill, args, params }) => {
      const skillDef = getSkillFn(skill);

      if (!skillDef) {
        const available = discoverSkillsFn().map((s) => s.name).join(', ');
...
/**
 * Skill invocation tool singleton using real registry functions.
 */
