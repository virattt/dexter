...
// Flat schema to avoid z.literal() which generates JSON Schema "const" keyword
// (not supported by Google Generative AI API)
const scheduleSchema = z.object({
  kind: z.enum(['at', 'every', 'cron']).describe("Schedule type: 'at'=one-shot, 'every'=interval, 'cron'=cron expression"),
  at: z.string().optional().describe("ISO-8601 timestamp for one-shot execution (kind=at)"),
  everyMs: z.number().min(60000).optional().describe('Interval in milliseconds, min 60000 (kind=every)'),
  anchorMs: z.number().optional().describe('Optional anchor timestamp in ms (kind=every)'),
  expr: z.string().optional().describe('Cron expression 5 or 6 fields (kind=cron)'),
  tz: z.string().optional().describe('IANA timezone, default system (kind=cron)'),
});
