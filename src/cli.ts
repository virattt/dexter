...
Do not submit anything until I explicitly confirm.`,
    '/theta-btc-weekly': `Give me optimal strike advice for BTC options expiring this Friday (same calendar day as Hypersurface) for BOTH secured puts AND covered calls. This is for execution on Hypersurface; tastytrade is data only — do not submit any order. Run tastytrade_theta_scan twice: (1) underlyings_csv=IBIT (or BITO), min_dte=1, max_dte=7, strategy_type=cash_secured_put; (2) same params but strategy_type=covered_call. Show each table_summary. Then output: "For secured puts we suggest strike $X" and "For covered calls we suggest strike $Y" with one-line rationale each. If IBIT is not in THETA-POLICY allowed underlyings, tell the user to add IBIT (or BITO/GBTC) to ~/.dexter/THETA-POLICY.md and rerun.`,
    '/hypersurface': `Advice for Hypersurface only — I will execute manually on Hypersurface; do not place any broker orders. Give me optimal strike advice for BTC options expiring this Friday (same calendar as Hypersurface weekly) for BOTH secured puts AND covered calls. Run tastytrade_theta_scan twice: (1) underlyings_csv=IBIT (or BITO), min_dte=1, max_dte=7, strategy_type=cash_secured_put; (2) same params but strategy_type=covered_call. Show each table_summary. Then output exactly in this format: "For secured puts we suggest strike $X" and "For covered calls we suggest strike $Y" with one-line rationale each (e.g. APR vs probability trade-off). If IBIT is not in THETA-POLICY allowed underlyings, tell me to add IBIT to ~/.dexter/THETA-POLICY.md and rerun; do not guess strikes.`,
