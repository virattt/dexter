/**
 * LangSmith Evaluation Runner for Yassir
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Yassir agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `yassir-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'yassir-finance-eval';
    ...
    const experimentName = `yassir-eval-${Date.now().toString(36)}`;
