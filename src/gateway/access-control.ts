...
function pairingPath(): string {
  return (
    process.env.YASSIR_PAIRING_PATH ??
    yassirPath('pairing', 'whatsapp.json')
  );
}
...
export function buildPairingReply(code: string, senderId: string): string {
  return [
    'Yassir access request received.',
    `Sender ID: ${senderId}`,
    `Approval code: ${code}`,
    'Ask the operator to approve this code in Yassir gateway config.',
  ].join('\n');
}
