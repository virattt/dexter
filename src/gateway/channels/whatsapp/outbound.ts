...
function debugLog(msg: string) {
  try {
    const logDir = kabutoPath('debug', 'logs');
    const logPath = kabutoPath('debug', 'logs', 'gateway-outbound.log');
...
function getActive(accountId?: string): ActiveListener {
  ...
  if (!first) {
    throw new Error('No active WhatsApp listener. Run kabuto gateway run.');
  }
  return first;
}
