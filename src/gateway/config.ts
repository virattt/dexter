...
const DEFAULT_GATEWAY_PATH = yassirPath('gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return overridePath ?? process.env.YASSIR_GATEWAY_CONFIG ?? DEFAULT_GATEWAY_PATH;
}
...
export function resolveWhatsAppAccount(
  ...
  const authDir = account.authDir ?? yassirPath('credentials', 'whatsapp', accountId);
