...
const DEFAULT_GATEWAY_PATH = kabutoPath('gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return overridePath ?? process.env.KABUTO_GATEWAY_CONFIG ?? DEFAULT_GATEWAY_PATH;
}
...
export function resolveWhatsAppAccount(
  ...
  const authDir = account.authDir ?? kabutoPath('credentials', 'whatsapp', accountId);
