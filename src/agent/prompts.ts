...
- **For price forecasts using prediction market signals** — when the user asks "what will X trade at in N days/weeks/months?", "price target for X", or "where is X headed?" — use polymarket_forecast. Polymarket hosts markets from 1 day to 12 months, so this tool works for short, medium, and longer horizons. It applies a research-backed weighted ensemble (Polymarket signals + sentiment + fundamentals + options skew) to produce a forecast price, 95% confidence interval, and a conviction grade. Signal quality is highest for 1–90 day horizons; longer horizons are supported but produce wider CIs. For richer results, call social_sentiment and get_financials first and pass their outputs via sentiment_score and fundamental_return. Do NOT use this for real-time prices (use get_market_data) or multi-year DCF valuation (use dcf skill).
