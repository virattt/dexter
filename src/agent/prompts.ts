...
- **For near-term price forecasts (1–14 days)** using prediction market probabilities — when the user asks "what will X trade at in N days?", "price target for X", or "where is X headed short-term?" — use polymarket_forecast. This tool applies a research-backed weighted ensemble (Polymarket signals + sentiment + fundamentals + options skew) to produce a forecast price, 95% confidence interval, and a conviction grade. For richer results, call social_sentiment and get_financials first and pass their outputs via sentiment_score and fundamental_return. Do NOT use this for long-term valuation (use dcf skill instead) or for real-time prices (use get_market_data).
