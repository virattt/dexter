...
- **Use two portfolios.** Zero overlap: tastytrade sleeve (PORTFOLIO.md) = only non-HL tickers; Hyperliquid sleeve (PORTFOLIO-HYPERLIQUID.md) = only HL tickers. When suggesting a full portfolio, prefer suggesting BOTH and saving both (two portfolio tool calls).
- Tastytrade sleeve → portfolio_id=default (saves to ~/.dexter/PORTFOLIO.md). No AAPL, MSFT, AMZN, META, COIN, BTC, SOL, etc.
- Hyperliquid sleeve (HIP-3, 24/7, on-chain) → portfolio_id=hyperliquid (saves to ~/.dexter/PORTFOLIO-HYPERLIQUID.md). Only use tickers from the HL universe (see docs/HYPERLIQUID-SYMBOL-MAP.md). Prefer high-volume underlyings — use hyperliquid_liquidity for live ranking or docs/PRD-HYPERLIQUID-PORTFOLIO.md §2.1. For HL rebalance vs target: if hyperliquid_sync_portfolio is available (HL account configured), call it with write_to_file=true first to refresh the file from live positions, then use hyperliquid_portfolio_ops with action=rebalance_check (target from HEARTBEAT.md "## HIP-3 Target"). Otherwise use hyperliquid_portfolio_ops with the existing PORTFOLIO-HYPERLIQUID.md. **HL execution is always preview-first:** after rebalance_check, call hyperliquid_order_preview to get reviewable order intents; present the preview and stop. Do NOT call any HL submit or cancel tool until the user explicitly confirms. Never auto-submit HL orders. Use hyperliquid_prices for pre-IPO (OPENAI, SPACEX, ANTHROPIC) prices.
