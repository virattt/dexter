...
| **tastytrade_theta_scan** | Find theta opportunities that match policy and constraints. | Underlying(s) or watchlist, strategy type (covered_call, cash_secured_put, credit_spread, iron_condor), DTE range, delta range, min credit, max buying power, exclude_earnings (bool). | Ranked list of candidate trades: underlying, strategy, strikes, credit, max loss, breakevens, DTE, delta; portfolio-fit note (e.g. "within policy," "would add 3% to NVDA exposure"). |
| **tastytrade_strategy_preview** | Produce a full trade memo and dry-run result without submitting. | Candidate trade (underlying, strategy, legs) or explicit leg list. | Trade memo: thesis, setup type, legs, credit/debit, max profit/loss, breakevens, buying power effect, exit plan, invalidation, roll plan; dry-run result if order flow enabled. |
...
| **Policy enforcement** | theta_scan and strategy_preview filter or warn using THETA-POLICY.md. Agent must not recommend trades that violate policy. |
...
| **Earnings/event filter** | When exclude_earnings or event filter is set, scan excludes underlyings with earnings in the window (using financial_search or calendar data). |
