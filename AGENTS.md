...
- Repo: https://github.com/oden41/kabuto (forked from https://github.com/virattt/dexter)
- Kabuto is a CLI-based AI agent for deep financial research on the Japanese stock market, built with TypeScript, Ink (React for CLI), and LangChain.
...
- Source code: `src/`
  ...
  - Finance tools: `src/tools/finance/` (prices, fundamentals, filings, etc.)
...
- Config: `.kabuto/settings.json` (persisted model/provider selection)
...
- Supported: OpenAI, Anthropic (default for Kabuto), Google, xAI (Grok), OpenRouter, Ollama (local).
...
- `read_filings`: filing reader for regulatory documents.
...
- Finance: `JQUANTS_API_KEY`, `EDINET_API_KEY`
...
- Config stored in `.kabuto/settings.json` (gitignored).
